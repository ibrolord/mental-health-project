...
export function Navigation() {
  ...
  const navItems = [
    ...
    { href: '/accountability', label: 'Together', icon: '🤝' },
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="md:hidden">
            <Link href="/settings" className={buttonVariants({ variant: 'ghost', size: 'sm' })} aria-label="Settings">
                ⚙️
            </Link>
          </div>
        </div>
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-2 py-2">
          <div className="flex justify-around">
            {navItems.slice(0, 5).map((item) => (
              <Link key={item.href} href={item.href} className={cn(buttonVariants({ variant: pathname === item.href ? 'default' : 'ghost', size: 'sm' }), 'flex h-auto flex-col py-2')}>
