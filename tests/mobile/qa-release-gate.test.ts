...
describe('exhaustive mobile QA release gate', () => {
  it('covers every native route file and has unique evidence rows', () => {
    ...
    expect(checklist.expectedInventory).toEqual({ routes: 33, routeChecks: 670, workflows: 110, total: 780 });
    expect(checklist.routes).toHaveLength(33);
    expect(checklist.workflows).toHaveLength(110);
    expect(ids).toHaveLength(780);
