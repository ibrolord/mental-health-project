-- Bind every check-in to the user who initiated it. Attribution is read from
-- client storage before this RPC, so the authenticated session can otherwise
-- change between the tap and the transaction.
...
DROP FUNCTION IF EXISTS public.save_check_in_with_attribution(
  public.mood_emoji,
  TEXT,
  TEXT[],
  DATE,
...
CREATE FUNCTION public.save_check_in_with_attribution(
...
COMMENT ON FUNCTION public.save_check_in_with_attribution(
  ...
  'Atomically stores a check-in only when the initiating owner still matches auth.uid().';
